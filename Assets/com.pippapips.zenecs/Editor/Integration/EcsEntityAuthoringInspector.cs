...
{
    [CustomEditor(typeof(EcsEntityAuthoring))]
    public sealed class EcsEntityAuthoringInspector : UnityEditor.Editor
    {
        ReorderableList _list;
        SerializedProperty _drafts;

        const string PREF_DISALLOW_DUP = "ZenECS.Authoring.DisallowDuplicates";
        const string PREF_GROUP_FILTER = "ZenECS.Authoring.GroupFilter";
        ...
        void OnEnable()
        {
            ...
            _drafts = serializedObject.FindProperty("_drafts");
            ...
            _list = new ReorderableList(serializedObject, _drafts, true, true, true, true);
            _list.drawHeaderCallback = DrawHeader;
            _list.drawElementCallback = DrawElement;
            _list.elementHeightCallback = i => CalcElementHeight(_drafts, i);
                ...
                EditorApplication.delayCall += () =>
                {
                    ...
                    ComponentPickerPopup.Open(
                        ...
                        picked =>
                        {
                            ...
                            _drafts.arraySize++;
                            var el = _drafts.GetArrayElementAtIndex(_drafts.arraySize - 1);
                        ...
                        favs: _favStore,
                        recents: _recents,
                        isDisabled: (Type t) => _disallowDup && HasDup(t, -1),   // ★ 비활성 규칙
                        disabledHint: " (이미 추가됨)"                           // 표시 힌트
                    );
                };
            };
            ...
            {
                if (list.index >= 0 && list.index < _drafts.arraySize)
                {
                    _drafts.DeleteArrayElementAtIndex(list.index);
                    serializedObject.ApplyModifiedProperties();
                }
            };
        }
        ...
        void DrawHeader(Rect _)
        {
            ...
            {
                GUILayout.Label("Draft Components", EditorStyles.boldLabel, GUILayout.Width(140));
                ...
                // 그룹 필터가 활성화되어 있으면 배지 + 초기화 버튼 노출
                if (!string.IsNullOrEmpty(_groupFilter) && _groupFilter != "(All)")
                {
                    //GUILayout.Space(6);
                    //GUILayout.Label($"Filtering: {_groupFilter}", EditorStyles.miniBoldLabel);
        ...
        {
            var el = _drafts.GetArrayElementAtIndex(index);
            var typeProp = el.FindPropertyRelative("TypeName");
            ...
            if (GUI.Button(rPick, "Select"))
            {
                ...
                EditorApplication.delayCall += () =>
                {
                    // Select: 자기 항목(index)은 허용, 다른 항목에 이미 있으면 비활성화
                    ComponentPickerPopup.Open(
                        screenRect,
                        picked =>
                        {
                        ...
                        favs: _favStore,
                        recents: _recents,
                        isDisabled: (Type tt) => _disallowDup && HasDup(tt, index), // ★ 현재 인덱스 제외
                        disabledHint: " (이미 추가됨)"
                    );
                };
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            var author = (EcsEntityAuthoring)target;
            EditorGUILayout.Space(2);

            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.LabelField($"EntityId: {(author ? GetPrivate(author, "_entityId") : 0)}", EditorStyles.miniBoldLabel);
                GUILayout.FlexibleSpace();
                EditorGUILayout.LabelField("Play 시 OnEnable → Draft 적용 & EntityView로 치환", EditorStyles.miniLabel, GUILayout.Width(320));
            }
        ...
        static int GetPrivate(EcsEntityAuthoring t, string field)
        {
            var f = typeof(EcsEntityAuthoring).GetField(field, BindingFlags.Instance | BindingFlags.NonPublic);
            return f != null ? (int)f.GetValue(t) : 0;
        }
        ...
        {
            for (int i = 0; i < _drafts.arraySize; i++)
            {
                if (i == ignoreIndex) continue;
                var el = _drafts.GetArrayElementAtIndex(i);
        ...
        void RemoveDuplicatesKeepingFirst()
        {
            ...
            for (int i = _drafts.arraySize - 1; i >= 0; i--)
            {
                var el = _drafts.GetArrayElementAtIndex(i);
                var t = Resolve(el.FindPropertyRelative("TypeName").stringValue);
                if (t == null) continue;
                if (seen.Contains(t))
                    _drafts.DeleteArrayElementAtIndex(i);
                else
                    seen.Add(t);
            }
            serializedObject.ApplyModifiedProperties();
        ...
        {
            const string PREF = "ZenECS.Authoring.Favorites";
        ...
        {
            const string PREF = "ZenECS.Authoring.Recents";
            const int MAX = 20;
        ...
        sealed class ComponentPickerPopup : EditorWindow
        {
            ...
            Func<Type, bool> _isDisabled;     // ★ 추가: 비활성 규칙
